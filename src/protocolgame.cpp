...
bool ProtocolGame::parseFirstPacket(NetworkMessage& msg)
{
	...
	Dispatcher::getInstance().addTask(createTask(boost::bind(
		&ProtocolGame::login, this, character, accnumber, password, operatingSystem, version, gamemaster)));
	return true;
}

void ProtocolGame::parsePacket(NetworkMessage &msg)
{
	if(!player || !m_acceptPackets || g_game.getGameState() == GAME_STATE_SHUTDOWN || msg.size() <= 0)
		return;
...
void ProtocolGame::sendAddCreature(const Creature* creature, const Position& pos)
{
	if(!canSee(creature))
		return;
	...
	if(creature != player)
	{
		AddTileCreature(msg, pos, creature);
		return;
	}
	...
	uint32_t newStackpos, const Tile* oldTile, const Position& oldPos, uint32_t oldStackpos, bool teleport)
{
	...
	else if(canSee(oldPos) && canSee(newPos))
	{
		...
		if(msg)
		{
			...
			{
				RemoveTileItem(msg, oldPos, oldStackpos);
				AddTileCreature(msg, newPos, creature);
			}
		}
	}
...
void ProtocolGame::AddCreature(NetworkMessage_ptr msg, const Creature* creature, bool known, uint32_t remove)
{
	...
	msg->put<char>((player->hasCustomFlag(PlayerCustomFlag_HasFullLight) ? 0xD7 : lightInfo.color));
